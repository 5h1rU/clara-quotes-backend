...
class SubmissionServiceTest {
    ...
    void cover() { quote.selectCoverage(CoverageType.BASIC, null, new BigDecimal("50.00"), clock.instant()); }
    @Test void submitsAndRepeatedSubmissionDoesNotRepeatSideEffects() {
        cover();
        ...
        assertThatThrownBy(() -> quote.selectCoverage(CoverageType.PREMIUM, null, new BigDecimal("200"), clock.instant()))
            .isInstanceOf(InvalidQuoteStateException.class);
    }
    @Test void failedSubmissionCanRetryWithoutLosingCoverage() {
        cover(); doThrow(new InsurerUnavailableException("timeout")).doNothing().when(insurer).submit(quote.getId());
        assertThatThrownBy(() -> service.submit(quote.getId())).isInstanceOf(InsurerUnavailableException.class);
        assertThat(quote.getStatus()).isEqualTo(QuoteStatus.SUBMISSION_FAILED);
        verifyNoInteractions(outbox);
        assertThat(service.submit(quote.getId()).status()).isEqualTo(QuoteStatus.SUBMITTED);
        verify(outbox).save(any(OutboxEvent.class));
    }
    @Test void incompleteQuoteNeverCallsTheInsurer() {
        assertThatThrownBy(() -> service.submit(quote.getId())).isInstanceOf(InvalidQuoteStateException.class);
