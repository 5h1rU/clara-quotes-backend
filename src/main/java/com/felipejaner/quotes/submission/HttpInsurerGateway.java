...
public class HttpInsurerGateway implements InsurerGateway {
    ...
    public HttpInsurerGateway(@Value("${quotes.insurer.url}") URI endpoint,
    ...
    public void submit(UUID quoteId) {
        ...
        var request = HttpRequest.newBuilder(endpoint).timeout(timeout)
                .header("Idempotency-Key", quoteId.toString()).GET().build();
        try {
            var response = client.send(request, HttpResponse.BodyHandlers.discarding());
            if (response.statusCode() < 200 || response.statusCode() >= 300)
                throw new InsurerUnavailableException("The insurer returned HTTP " + response.statusCode() + ". Your quote is saved; retry submission.");
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new InsurerUnavailableException("Submission was interrupted. Your quote is saved; retry submission.");
        } catch (IOException e) {
            throw new InsurerUnavailableException("The insurer could not be reached in time. Your quote is saved; retry submission.");
        }
    }
}
