...
public class SubmissionService {
    ...
    public SubmissionService(QuoteRepository quotes, InsurerGateway insurer, OutboxRepository outbox,
                             ApplicationEventPublisher events, Clock clock) {
        this.quotes = quotes; this.insurer = insurer; this.outbox = outbox; this.events = events; this.clock = clock;
    }
    ...
    public QuoteResponse submit(UUID id) {
        Quote quote = quotes.findForUpdate(id).orElseThrow(() -> new QuoteNotFoundException("Quote was not found."));
