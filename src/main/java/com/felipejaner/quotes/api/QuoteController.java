...
public class QuoteController {
  ...
  public QuoteController(QuoteService quotes, SubmissionService submissions) {
    this.quotes = quotes;
  ...
  @GetMapping("/{id}")
  QuoteResponse get(@PathVariable UUID id) {
    return quotes.get(id);
  }

  @PatchMapping("/{id}/coverage")
  QuoteResponse coverage(@PathVariable UUID id, @Valid @RequestBody CoverageRequest request) {
    return quotes.updateCoverage(id, request);
  }

  @PostMapping("/{id}/submit")
  QuoteResponse submit(@PathVariable UUID id) {
