...
public class InfrastructureConfig {
    ...
    @Bean NewTopic quoteTopic(@Value("${quotes.kafka.topic}") String topic) {
        return TopicBuilder.name(topic).partitions(1).replicas(1).build();
    }
    @Bean CacheInvalidation cacheInvalidation(CacheManager manager) { return new CacheInvalidation(manager); }
    public static class CacheInvalidation {
        private final CacheManager manager;
        CacheInvalidation(CacheManager manager) { this.manager = manager; }
        @TransactionalEventListener(phase = TransactionPhase.AFTER_COMMIT)
        public void onChange(QuoteChanged event) {
            var cache = manager.getCache("quotes");
            if (cache != null) {
                if (event.id() == null) cache.clear(); else cache.evict(event.id());
            }
        }
    }
}
