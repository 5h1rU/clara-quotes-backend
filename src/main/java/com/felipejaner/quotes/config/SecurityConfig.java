...
public class SecurityConfig {
  ...
  UserDetailsService users(
      @Value("${quotes.auth.username}") String username,
      @Value("${quotes.auth.password}") String password) {
        ...
        (org.springframework.security.web.AuthenticationEntryPoint)
            (request, response, exception) -> {
              ...
              mapper.writeValue(
                  ...
                  ApiError.of("UNAUTHORIZED", "Sign in with the configured API credentials."));
            };
                ...
                e.authenticationEntryPoint(entryPoint)
                    .accessDeniedHandler(
                        (request, response, ex) -> {
                          ...
                          mapper.writeValue(
                              response.getOutputStream(),
                              ApiError.of("FORBIDDEN", "You cannot perform this action."));
                        }))
        .build();
  }
  ...
  CorsConfigurationSource corsConfigurationSource(
      @Value("${quotes.cors.allowed-origin}") String origin) {
