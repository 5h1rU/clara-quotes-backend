...
@Configuration
@EnableScheduling
@ConditionalOnProperty(name = "quotes.scheduling.enabled", havingValue = "true", matchIfMissing = true)
public class ScheduledJobs {
    private final DraftExpirationService expiration;
    private final OutboxPublisher outbox;
    public ScheduledJobs(DraftExpirationService expiration, OutboxPublisher outbox) { this.expiration = expiration; this.outbox = outbox; }
    @Scheduled(fixedDelayString = "${quotes.expiration.interval}")
    public void expireDrafts() { expiration.expire(); }
    @Scheduled(fixedDelayString = "${quotes.outbox.interval}")
    public void publishEvents() { outbox.publishPending(); }
}
